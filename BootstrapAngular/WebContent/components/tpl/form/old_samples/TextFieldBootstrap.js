...
.directive('textFieldBootstrap', function () {
    ...
    return {
        ...
        template: '<div class="form-group" ng-class="{ \'has-error\' : field.$invalid }"> ' +
                    ...
                    '<input ng-readonly="ro" type="text"  class="form-control" mask="{{fmt}}"' + 
                    	'name="{{internalName}}" id="{{internalName}}" ng-model="model" ng-required="req"> ' +
                    '<div ng-messages="field.$error">'+
	            		'<div ng-message="required" class="help-block">O campo \'{{name}}\' é de preenchimento obrigatório.</div>'+
	            		'<div ng-message="mask" class="help-block">Preencha o campo \'{{name}}\' no formato \'{{fmt}}\'.</div>'+
	            	'</div>' +
                  '</div>',
